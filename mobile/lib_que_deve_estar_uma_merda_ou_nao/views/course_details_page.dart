...
class _CourseDetailsPageState extends State<CourseDetailsPage> {
  ...
  List<Map<String, dynamic>> _topicosGlobais = [];
  List<Map<String, dynamic>> _licoes = [];
  Map<int, List<Map<String, dynamic>>> _materiaisPorLicao = {};
  ...
  Future<Map<String, dynamic>?> _fetchCourseDetails() async {
    final id = widget.id;
    print('DEBUG: _fetchCourseDetails called for id: $id');
    // 1. Tenta buscar localmente
    final localList = await dbHelper.listarCursos();
    print('DEBUG: localList: ' + localList.toString());
    final localCourse = localList.where((c) => c['idcurso'] == id).isNotEmpty
        ? localList.firstWhere((c) => c['idcurso'] == id)
        : null;
    print('DEBUG: localCourse: ' + localCourse.toString());
    if (localCourse != null) {
      // Fetch tópicos do curso
      final topicos = await dbHelper.listarTopicosDoCurso(id);
      print('DEBUG: topicos: ' + topicos.toString());
      // Fetch licoes e materiais
      final licoes = await dbHelper.listarLicoesDoCurso(id);
      print('DEBUG: licoes: ' + licoes.toString());
      // Garante que cada licao tem sempre uma lista de materiais
      for (final licao in licoes) {
        final materiais = await dbHelper.listarMateriaisDaLicao(licao['idlicao']);
        licao['materiais'] = materiais ?? [];
        print('DEBUG: materiais da licao \\${licao['idlicao']}: ' + licao['materiais'].toString());
      }
      // Fetch categoria
      Map<String, dynamic>? categoriaData;
      if (localCourse['idcategoria'] != null && localCourse['idcategoria'].toString().isNotEmpty) {
        final categorias = await dbHelper.listarCategorias();
        print('DEBUG: categorias: ' + categorias.toString());
        categoriaData = categorias.firstWhere(
          (cat) => cat['idcategoria'] == localCourse['idcategoria'],
          orElse: () => {},
        );
        print('DEBUG: categoriaData: ' + categoriaData.toString());
      } else {
        categoriaData = null;
      }
      // Fetch area (primeira área da categoria, se existir)
      Map<String, dynamic>? areaData;
      if (categoriaData != null && categoriaData['idcategoria'] != null) {
        final areas = await dbHelper.listarAreas(idCategoria: categoriaData['idcategoria']);
        print('DEBUG: areas: ' + areas.toString());
        if (areas.isNotEmpty) {
          areaData = areas.first;
        } else {
          areaData = null;
        }
        print('DEBUG: areaData: ' + (areaData?.toString() ?? 'null'));
      } else {
        areaData = null;
      }
      // Fetch topicos globais (da area, se existir)
      List<Map<String, dynamic>> topicosGlobais = [];
      if (areaData != null && areaData['idarea'] != null) {
        topicosGlobais = await dbHelper.listarTopicosGlobais(idArea: areaData['idarea']);
        print('DEBUG: topicosGlobais: ' + topicosGlobais.toString());
      }
      if (mounted) {
        setState(() {
          _isInscrito = localCourse['inscrito'] == 1 || localCourse['inscrito'] == true;
          _courseData = {
            ...localCourse,
            'disponivel': localCourse['disponivel'] == 1 || localCourse['disponivel'] == true,
            'topicos': topicos,
            'licoes': licoes, // licoes já têm materiais aninhados
          };
          _categoryData = categoriaData;
          _areaData = areaData;
          _topicosGlobais = topicosGlobais;
        });
      }
      return {
        ...localCourse,
    ...
    // 2. Se não encontrar localmente, busca da API
    print('DEBUG: localCourse is null, fetching from API...');
    final data = await servidor.getData('curso/$id');
    print('DEBUG: data from API: ' + data.toString());
    ...
    print('DEBUG: data from API is not a Map<String, dynamic>');
    return null;
  }
            ...
            children: [
              if (_categoryData != null && _categoryData!.isNotEmpty)
                Padding(
                  ...
                  child: Text(
                    'Categoria:  a0${_categoryData?['designacao'] ?? '-'}',
                    style: const TextStyle(fontSize: 16, color: Color(0xFF007BFF), fontWeight: FontWeight.w600),
                  ),
                ),
              if (_areaData != null && _areaData!.isNotEmpty)
                Padding(
                  padding: const EdgeInsets.only(bottom: 8.0),
                  child: Text(
                    'Área:  a0${_areaData?['designacao'] ?? '-'}',
                    style: const TextStyle(fontSize: 16, color: Color(0xFF007BFF), fontWeight: FontWeight.w600),
                  ),
                ),
              if (_topicosGlobais.isNotEmpty)
                Padding(
                  padding: const EdgeInsets.only(bottom: 8.0),
                  child: Text(
                    'Tópicos globais: ' + _topicosGlobais.map((t) => t['designacao']).join(', '),
                    style: const TextStyle(fontSize: 15, color: Color(0xFF007BFF)),
                  ),
                ),
