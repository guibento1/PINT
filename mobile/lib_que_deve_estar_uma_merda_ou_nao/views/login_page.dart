...
import '../backend/database_helper.dart'; // Add this import
...
class _LoginPageState extends State<LoginPage> {
  ...
  final DatabaseHelper _dbHelper = DatabaseHelper(); // Add this instance
  ...
  Future<void> _login() async {
    ...
    if (token != null) {
      ...
      if (userId != null) {
        ...
        print('Perfil recebido: $perfilResp');
        ...
        if (perfil.isNotEmpty) {
          await _dbHelper.upsertPerfil({
            'idutilizador': int.tryParse(userId) ?? userId,
            'nome': perfil['nome'] ?? '',
            'email': perfil['email'] ?? '',
            'foto': perfil['foto'] ?? '',
        ...
        if (subscriptions != null && subscriptions.isNotEmpty) {
          ...
          if (userIdInt != null) {
            for (var topicId in subscriptions) {
              ...
              // Save notification subscription locally
              await _dbHelper.saveNotificationSubscription(userIdInt, topicId);
            }
          } else {
            print('Erro: userId não é um inteiro válido para subscrição de notificações.');
          }
        }
        if (mounted) {
          setState(() {
      ...
      print('Login falhou. result: $result');
      setState(() {
