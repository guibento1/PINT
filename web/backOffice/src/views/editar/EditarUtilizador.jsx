...
const EditarUtilizador = () => {
    ...
    const fetchUserData = useCallback(async () => {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || "Não foi possível carregar os dados do utilizador.";
    ...
    const handleSubmit = async (e) => {
        ...
        // Helper para adicionar ao info apenas se o valor for diferente e não vazio
        const addIfChangedAndNotEmpty = (key, newValue, originalValue) => {
            if (newValue !== originalValue) {
                // Se o novo valor for uma string vazia, envie-o como null,
                // caso contrário, envie o novo valor.
                // Isso é crucial para campos que podem ser nulos no DB mas têm check constraints.
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || "Ocorreu um erro ao atualizar o utilizador. Tente novamente.";
    ...
    const confirmUnenroll = async () => {
        ...
        setCourseActionLoading(true);
        try {
            await api.post(`/curso/${courseToActUpon.idcurso}/sair`, {  utilizador: id  });
            setCourseActionStatus(0);
            setCourseActionMessage(`Utilizador desinscrito de "${courseToActUpon.nome}" com sucesso!`);
            fetchEnrolledCourses();
        } catch (err) {
            console.error("Erro ao desinscrever utilizador do curso:", err);
            setCourseActionStatus(1);
            setCourseActionMessage(err.response?.data?.message || `Erro ao desinscrever de "${courseToActUpon.nome}".`);
        } finally {
            setCourseActionLoading(false);
    ...
    if (loading) {
        return (
            <div className="container mt-5 text-center">
                ...
                <p className="mt-2 text-muted">Carregando dados do utilizador e cursos...</p>
            </div>
        );
    }
        ...
        return (
            <div className="container mt-5">
                ...
                <div className="text-center mt-3">
                    <button className="btn btn-primary" onClick={() => navigate('/gerir-utilizadores')}>
    ...
    return (
        <>
            <Modal isOpen={isGlobalModalOpen} onClose={closeGlobalModal} title={globalModalTitle}>
                {globalModalBody}
                <div className="d-flex justify-content-end mt-4">
                    <button type="button" className="btn btn-primary" onClick={globalModalTitle === "Sucesso" ? closeGlobalModalAndRedirect : closeGlobalModal}>
                        OK
                    </button>
                </div>
            </Modal>

            <Modal isOpen={isCourseActionModalOpen} onClose={closeCourseActionModal} title="Confirmar Desinscrição">
                <p>Tem a certeza que deseja **desinscrever** o utilizador **"{userData.nome}"** do curso **"{courseToActUpon?.nome}"**?</p>
                                ...
                                <div className="mb-4">
                                    ...
                                    {ALL_AVAILABLE_ROLES.length > 0 ? (
                                        ALL_AVAILABLE_ROLES.map(roleName => (
                                            <div key={roleName} className="form-check form-check-inline">
                                                <input
                            ...
                            {courseActionLoading ? (
                                <div className="text-center">
                                    <div className="spinner-border text-info" role="status">
                                        <span className="visually-hidden">Carregando cursos...</span>
                                    </div>
                                    <p className="mt-2 text-muted">Carregando cursos inscritos...</p>
                                </div>
                                ...
                                <p className="text-center text-muted">Este utilizador não está inscrito em nenhum curso.</p>
                            )}
                        </div>
                    </div>
                </div>
