...
export default function EditarArea() {
    ...
    useEffect(() => {
        const fetchAreaData = async () => {
            ...
            try {
                ...
                setError('Não foi possível carregar a área ou categorias. Tente novamente mais tarde.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                categoria: categoriaSelecionada ? parseInt(categoriaSelecionada, 10) : null
            };
            ...
            setOperationMessage(err.response?.data?.message || 'Erro ao atualizar área.');
        } finally {
            setSubmitting(false);
            openResultModal();
        }
    ...
    if (loading) {
        return <div className="container mt-4"><p>A carregar área...</p></div>;
    }
    ...
    return (
        <div className="container mt-4">
            ...
            <form onSubmit={handleSubmit}>
                <div className="mb-3">
                    <label htmlFor="designacaoArea" className="form-label">Designação da Área</label>
                    <input
                        type="text"
                ...
                <div className="mb-3">
                    <label htmlFor="categoriaArea" className="form-label">Categoria</label>
                    <select
                    ...
                    {categoriasDisponiveis.length === 0 && <div className="form-text text-danger">Nenhuma categoria disponível.</div>}
                </div>
                <button type="submit" className="btn btn-primary me-2" disabled={submitting}>
                    {submitting ? 'A Atualizar...' : 'Atualizar Área'}
                </button>
                <button type="button" className="btn btn-secondary" onClick={() => navigate('/gerir-estrutura')} disabled={submitting}>
                    Cancelar
                </button>
            </form>
