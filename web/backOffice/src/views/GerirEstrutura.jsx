...
export default function GerirEstrutura() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (
        ...
        throw new Error(
          "Ação de confirmação desconhecida ou dados incompletos."
        );
      }
    } catch (err) {
      ...
      const estruturaAninhada = await Promise.all(
        categoriasData.map(async (categoria) => {
          ...
          try {
                ...
                try {
                  ...
                  topicos = await Promise.all(
                    resTopicos.data.map(async (topico) => {
                      ...
                      try {
                        const resNobjetos = await api.get(
                          `/topico/id/${topico.idtopico}/nobjetos`
                        );
                        ...
                        console.error(
                          `Erro ao buscar número de objetos para o tópico ${topico.idtopico}:`,
                          nObjetosError
                        );
                        dependencias = null;
                      }
                  ...
                  console.error(
                    `Erro ao buscar tópicos para área ${area.idarea}:`,
                    topicoError
                  );
                }
                return {
            ...
            console.error(
              `Erro ao buscar áreas para categoria ${categoria.idcategoria}:`,
              areaError
            );
          }
          return {
  ...
  useEffect(() => {
    const fetchAndSetStructure = async () => {
      try {
        ...
        setError(
          "Não foi possível carregar a estrutura. Tente novamente mais tarde."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleEliminarTopico = (idCategoria, idArea, idTopico, event) => {
    event.stopPropagation();
    openConfirmModal("eliminarTopico", {
      idcategoria: idCategoria,
      idarea: idArea,
      idtopico: idTopico,
    });
    ...
    return (
      <div className="container mt-5">
        <div className="text-center my-5">
          ...
          <p className="mt-2 text-muted">A carregar estrutura...</p>
        </div>
      </div>
    );
  }
      ...
      {estruturaCompleta.length === 0 ? (
        ...
        <div className="accordion" id="estruturaAccordion">
          {estruturaCompleta.map((categoria) => (
            <div
              className="accordion-item rounded-3 mb-2"
              key={categoria.idcategoria}
            >
              <h2
                className="accordion-header d-flex align-items-center"
                id={`heading${categoria.idcategoria}`}
              >
                <button
                ...
                <div className="btn-group me-2" role="group">
                  ...
                  {categoria.areas.length === 0 && (
                    <button
                      ...
                      onClick={(e) =>
                        handleEliminarCategoria(categoria.idcategoria, e)
                      }
                    >
                      Eliminar
                    </button>
              ...
              >
                <div className="accordion-body">
                  ...
                  {categoria.areas && categoria.areas.length > 0 ? (
                    <div
                      className="accordion"
                      id={`areasAccordion${categoria.idcategoria}`}
                    >
                      {categoria.areas.map((area) => (
                        <div
                          className="accordion-item rounded-3 mb-1"
                          key={area.idarea}
                        >
                          <h3
                            className="accordion-header d-flex align-items-center"
                            id={`headingArea${area.idarea}`}
                          >
                            <button
                              className="accordion-button collapsed flex-grow-1"
                              type="button"
                                    ...
                                    >
                                      <div className="d-flex align-items-center flex-grow-1">
                                        ...
                                        <div className="d-flex align-items-center gap-3">
                                          ...
                                          {topico.dependencias?.nPosts !==
                                            undefined && (
                                            <span className="badge bg-info rounded-pill text-dark">
                                              <i className="ri-file-text-line me-1"></i>{" "}
                                              {topico.dependencias.nPosts} Posts
                                            </span>
                                          )}
                                        </div>
                                      </div>
                                      ...
                                      <div className="btn-group" role="group">
                                        <button
                                          ...
                                          className="btn btn-sm btn-outline-info me-2 rounded-pill"
                                          onClick={() =>
                                            handleEditTopico(topico.idtopico)
                                          }
                                        >
                                          Editar
                                        </button>
                                        {topico.dependencias &&
                                          topico.dependencias.nCursos === 0 &&
                                          topico.dependencias.nPosts === 0 && (
                                            <button
                                              type="button"
                                              className="btn btn-sm btn-outline-danger fw-semibold rounded-pill px-3"
                                              onClick={(e) =>
                                                handleEliminarTopico(
                                                  categoria.idcategoria,
                                                  area.idarea,
                                                  topico.idtopico,
                                                  e
          ...
          <div className="alert alert-info mt-4" role="alert">
            ...
            <strong>Atenção:</strong> Apenas é permitido eliminar Categorias,
            Áreas ou Tópicos quando não existirem dependências (Áreas, Tópicos,
            Cursos ou Posts) que os requeiram.
          </div>
        </div>
      )}
