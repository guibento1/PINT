...
export default function GerirUtilizadores() {
    ...
    const confirmAction = async () => {
        ...
        try {
            if (actionType === 'activate') {
                ...
                setOperationMessage(`Utilizador "${userToActUpon.nome}" ativado com sucesso!`);
            } else if (actionType === 'delete') {
                await deleteUtilizador(userToActUpon.idutilizador);
                setOperationStatus(0);
                setOperationMessage(`Utilizador "${userToActUpon.nome}" desativado/eliminado com sucesso!`);
            }
            fetchUtilizadores(); 
        } catch (err) {
            console.error(`Erro ao ${actionType} utilizador:`, err);
            setOperationStatus(1);
            setOperationMessage(err.response?.data?.message || `Erro ao ${actionType} utilizador "${userToActUpon.nome}".`);
        } finally {
            setProcessing(false);
    ...
    const getActionModalBody = () => {
        ...
        if (actionType === 'activate') {
            return <p>Tem a certeza que deseja **ativar** o utilizador **"{userToActUpon.nome}"**?</p>;
        }
        if (actionType === 'delete') {
            return <p>Tem a certeza que deseja **desativar/eliminar** o utilizador **"{userToActUpon.nome}"**?</p>;
        }
        return null;
    };
                        ...
                        <tbody>
                            {utilizadores.map((utilizador) => (
                                <tr key={utilizador.idutilizador}>
                                    ...
                                    <td>{new Date(utilizador.dataregisto).toLocaleDateString()}</td>
                                    <td>
                                        {utilizador.ativo ?
                                    ...
                                    <td>
                                        {utilizador.roles && utilizador.roles.length > 0 ?
                                            utilizador.roles.map(role => <span key={role.id} className="badge bg-info text-dark me-1">{role.role}</span>) :
                                    ...
                                    <td>
                                        ...
                                        {utilizador.ativo ? (
                                            ...
                                            <button
                                                ...
                                                onClick={() => handleActionClick(utilizador, 'activate')}
            ...
            >
                ...
                <div className="d-flex justify-content-end gap-2 mt-4">
                    ...
                    <button
                        ...
                        disabled={processing}
                    >
                        {processing ? 'A Processar...' : (actionType === 'activate' ? 'Ativar' : 'Desativar')}
                    </button>
                </div>
            </Modal>
